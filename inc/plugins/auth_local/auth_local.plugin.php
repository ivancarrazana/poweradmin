...
function authenticate_local() {
    ...
    if (isset($_POST["authenticate"])) {
        $_SESSION["userpwd"] = base64_encode(openssl_encrypt($_POST['password'], "aes-256-cbc",  md5($session_key), OPENSSL_RAW_DATA, substr(md5(md5($session_key)),16,16)));
...
function LDAPAuthenticate() {
    ...
    if (isset($_SESSION["userlogin"]) && isset($_SESSION["userpwd"])) {
        ...
        $session_pass = rtrim(openssl_decrypt(base64_decode($_SESSION["userpwd"]), "aes-256-cbc", md5($session_key), OPENSSL_RAW_DATA, substr(md5(md5($session_key)),16,16)) , "\0");;
